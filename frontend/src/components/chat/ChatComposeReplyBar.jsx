...
export default function ChatComposeReplyBar({ author, preview, onCancel }) {
  const headline = previewHeadline(preview);

  return (
    <div
      className="mb-2 flex items-start gap-2.5 border-b border-slate-200/90 pb-2.5 dark:border-white/10"
      data-testid="chat-compose-reply-bar"
    >
      ...
      <div className="min-w-0 flex-1">
        <p className="flex items-center gap-1.5 text-sm font-semibold text-primary">
          <svg viewBox="0 0 20 20" className="h-4 w-4 shrink-0" aria-hidden="true">
            <path
              fill="currentColor"
              d="M7.707 3.293a1 1 0 0 1 0 1.414L5.414 7H12a5 5 0 0 1 0 10h-1a1 1 0 1 1 0-2h1a3 3 0 1 0 0-6H5.414l2.293 2.293a1 1 0 1 1-1.414 1.414l-4-4a1 1 0 0 1 0-1.414l4-4a1 1 0 0 1 1.414 0Z"
            />
          </svg>
          {author ? `${author} ga javob` : "Javob berish"}
        </p>
        {headline ? (
          <p
            className="mt-1 line-clamp-2 break-words [overflow-wrap:anywhere] text-sm font-medium text-slate-500 dark:text-slate-400"
            title={preview}
          >
            {headline}
          </p>
