...
const UNIVERSITIES_CATALOG_PATH = "/universitetlar";
            ...
            {universities.map((university, index) => {
              ...
              return (
                <motion.article
                  ...
                  whileHover={{ y: -10 }}
                  className="group flex h-full flex-col overflow-hidden rounded-[2rem] border border-slate-200 bg-white shadow-soft transition hover:border-primary/30 dark:border-white/10 dark:bg-white/[0.06]"
                >
                  <Link to={publicPath} className="block focus-visible:outline-none">
                    <UniversityCampusBanner university={university} className="h-52 sm:h-56" />
                  </Link>
                  <div className="flex flex-1 flex-col p-6">
                    <Link to={publicPath} className="block focus-visible:outline-none">
                      <h3 className="text-2xl font-black text-slate-950 transition group-hover:text-primary dark:text-white">
                        {university.name}
                      </h3>
                    ...
                    <div className="mt-auto flex items-center justify-between border-t border-slate-100 pt-5 dark:border-white/10">
                      ...
                      <Link
                        to={publicPath}
                        className="text-sm font-black text-primary transition hover:underline"
                      >
                        Batafsil →
                      </Link>
                    </div>
