...
export default function CompareQuickPicks({ pairs, onApply, label }) {
  ...
  return (
    <div>
      <p className="text-center text-[10px] font-bold uppercase tracking-wide text-slate-400">{label}</p>
      <div className="mt-2 grid gap-2 sm:grid-cols-2">
        {pairs.map(({ ids, universities }) => (
          <button
            ...
            type="button"
            onClick={() => onApply(ids)}
            className="group relative flex min-w-0 items-center justify-center gap-2 rounded-xl bg-gradient-to-r from-slate-50 to-white px-3 py-2.5 text-center text-xs font-bold text-slate-700 ring-1 ring-slate-200/70 transition hover:-translate-y-0.5 hover:shadow-md hover:ring-primary/40 dark:from-white/[0.04] dark:to-white/[0.02] dark:text-slate-200 dark:ring-white/10"
          >
            ...
            <span
              className="absolute right-3 shrink-0 text-primary opacity-0 transition group-hover:opacity-100"
              aria-hidden="true"
            >
              →
            </span>
