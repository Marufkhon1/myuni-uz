...
function AspectBar({ label, value, icon, variant = "default" }) {
  ...
  if (variant === "inline") {
    return (
      <span
        className={`inline-flex shrink-0 items-center gap-0.5 rounded-full px-1.5 py-0.5 ring-1 ${ASPECT_CHIP_THEME.shell}`}
      ...
      >
        ...
        <span
          className={`shrink-0 text-[9px] font-black tabular-nums leading-none ${ASPECT_CHIP_THEME.score}`}
        >
          {scoreLabel}
        </span>
      </span>
  ...
  if (variant === "chip") {
    return (
      <div
        className={`flex w-full min-w-0 flex-col gap-0.5 rounded-lg px-1.5 py-1 ring-1 ${ASPECT_CHIP_THEME.shell}`}
        title={`${label}: ${scoreLabel}`}
      >
        <div className="flex min-w-0 items-center gap-1">
          ...
          <span className="min-w-0 truncate text-[9px] font-bold leading-snug text-slate-600 dark:text-slate-300">
            {label}
          </span>
          <span
            className={`ml-auto shrink-0 text-[9px] font-black tabular-nums leading-none ${ASPECT_CHIP_THEME.score}`}
          >
            {scoreLabel}
          </span>
        </div>
        <div className={`h-0.5 w-full overflow-hidden rounded-full ${ASPECT_CHIP_THEME.track}`}>
          <div
  ...
  return (
    <div className="min-w-0 rounded-xl bg-slate-950/[0.03] px-3 py-2.5 ring-1 ring-slate-950/10 dark:bg-white/[0.04] dark:ring-white/10">
      <div className="flex items-start justify-between gap-3">
        <span className="text-xs font-bold leading-snug text-slate-600 dark:text-slate-300">
          {icon ? (
...
export default function ReviewAspectRatings({
  ...
  if (variant === "chip") {
    return (
      <div className="grid w-full min-w-0 grid-cols-1 gap-1 sm:grid-cols-2 2xl:grid-cols-3">
        {REVIEW_ASPECTS.map((aspect) => (
          <AspectBar
            key={aspect.id}
            label={aspect.label}
